...
export const createStarter = () => {
  ...
  class ActionHandler {
     ...
     */
    async handleAction(action: any, next: Function, lockInstance: any) {
          ...
          const asyncFunc = (async () => {
            await action(
              async (syncAction: Action<any>) => {
                  const actionToHandle = Object.assign({}, syncAction, { source: op });
                  await this.handleAction(actionToHandle, next, innerLock);
              },
              this.getState,
              this.dependencies()
            );
